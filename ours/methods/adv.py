...
from ours import _OutputType, ImageTxtSample
from ours.utils.surrogates import DYPTransformerClipAttackVisionModel, DYPOpenClipAttackVisionModel
from ours.utils.SSA import SSA_CommonWeakness
...
class AdvGeneratedImage(BaseMethod):
    ...
    def __init__(self, method_id: str, img_dir: str, img_size: Tuple[int, int], lazy_mode: bool = True, target_text: str = "A Bomb.") -> None:
        ...
        clip1 = DYPTransformerClipAttackVisionModel("clip-vit-large-patch14", target_text=target_text).to("cuda")
        laion_clip = DYPOpenClipAttackVisionModel("hf-hub:laion/CLIP-ViT-B-32-laion2B-s34B-b79K", target_text).to("cuda")
        laion_clip2 = DYPOpenClipAttackVisionModel("CLIP-ViT-B-16-laion2B-s34B-b88K", target_text).to("cuda")
        laion_clip3 = DYPOpenClipAttackVisionModel("CLIP-convnext_large_d_320.laion2B-s29B-b131K-ft-soup", target_text).to("cuda")
        laion_clip4 = DYPOpenClipAttackVisionModel("CLIP-ViT-bigG-14-laion2B-39B-b160k", target_text).to("cuda")
        sig_clip = DYPOpenClipAttackVisionModel("ViT-SO400M-14-SigLIP-384", target_text, resolution=(384, 384)).to("cuda")
    ...
    def hash(self) -> str:
        payload = {
            "method_id": getattr(self, "method_id", None),
