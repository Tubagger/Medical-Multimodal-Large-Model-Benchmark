...
def _parse_openclip_name(name: str):
    ...
    if name.startswith("CLIP-"):
        # CLIP-ViT-B-32-laion2B-s34B-b79K
        parts = name.split("-")
        model_name = "-".join(parts[1:4])      # ViT-B-32
        pretrained = "_".join(parts[4:])       # laion2B_s34B_b79K
...
class DYPOpenClipAttackVisionModel(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        model_name_, pretrained_ = _parse_openclip_name(model_name)
    ...
    #     self,
    #     model_name: str,
    #     target_text="A image of a bomb",
    #     device=torch.device("cuda"),
    #     resolution=(224, 224),
    #     pretrained: str = None,                     # ✅ 新增
    # ):
    #     super().__init__()

    #     # ✅ 兼容你原来那种长名字：自动解析
    #     if pretrained is None and model_name.startswith("CLIP-"):
    #         model_name, pretrained = _parse_openclip_name(model_name)

    #     clip, _, preprocess = open_clip.create_model_and_transforms(
    #         model_name,
    #         pretrained=pretrained                  # ✅ 关键
    #     )
    #     self.tokenizer = open_clip.get_tokenizer(model_name)
    #     self.clip = clip
    #     self.target_text = target_text
    ...
    #         ]
    #     )
    #     print(f"finished initializing the model {model_name} (pretrained={pretrained})")
    #     self.name = model_name
